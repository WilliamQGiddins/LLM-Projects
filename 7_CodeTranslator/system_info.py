...
def go_toolchain_info():
    """
    Return a dict with Go-related settings:
      - presence/path/version of the `go` tool
      - key values from `go env` (GOROOT, GOPATH, GOOS, GOARCH, GOMODCACHE)
      - relevant environment variables (GOROOT, GOPATH, GO111MODULE, GOTOOLDIR)
      - simple execution examples
    Uses the existing helpers: _run, _which, _first_line.
    """
    info = {
        "installed": False,
        "go": {
        ...
        "env": {
            ...
            "GO111MODULE": os.environ.get("GO111MODULE", ""),
            "GOTOOLDIR": os.environ.get("GOTOOLDIR", ""),
            "GOOS": "",
            "GOARCH": "",
            "GOMODCACHE": "",
        },
    ...
    go_path = _which("go")
    info["go"]["path"] = go_path or ""

    if go_path:
        # Basic version string, e.g. "go version go1.21.3 windows/amd64"
        info["go"]["version"] = _first_line(_run([go_path, "version"]))

        # Parse `go env` output: lines like KEY="VALUE"
        env_text = _run([go_path, "env"])
        env_lines = env_text.splitlines() if env_text else []
        parsed_env = {}
        ...
        # Fill in key fields from parsed `go env` (if available)
        for key in ("GOROOT", "GOPATH", "GOOS", "GOARCH", "GOMODCACHE"):
            if parsed_env.get(key):
                info["env"][key] = parsed_env[key]

        # Execution examples that should work on this system
        examples = [
            f'"{go_path}" version',
            f'"{go_path}" env',
            f'"{go_path}" build ./...',
        ...
        info["execution_examples"] = examples
